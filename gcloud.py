...
class GcsClient(object):
    ...
    def project(project_id=None):
        ...
        if GcsClient.client is None or not GcsClient.client.project:
            raise RuntimeError(
                "Google Cloud Project is undefined. use colab_utils.gcloud.config_project(project_id)"
            )
...
def gsutil_ls(bucket_name, filter=None, project_id=None):
  ...
  """
    ...
    try:
        ...
        files = [
            "{}{}".format(bucket_path, f.name) for f in bucket.list_blobs()
        ]
        if filter:
        ...
        return [
            ...
            "GCS bucket not found, path={}".format(bucket_path)
        ]
    except Exception as e:
        raise e


def gsutil_mb(bucket_name,
              gcs_class="regional",
  ...
  """
    ...
    if "BucketNotFoundException" in result:
        ...
        cmd = _cmd["make_bucket"].format(project_id, gcs_class, gcs_location,
                                         BUCKET_PATH)
        # print(cmd)
        ...
        if len(result) == 1 and "Creating gs://" in result[0]:
            return BUCKET_PATH
        raise RuntimeError(
            "ERROR: problem creating bucket, bucket={}".format(BUCKET_PATH))
    else:
        raise ValueError(
            "ERROR: gcs bucket exists, bucket={}".format(BUCKET_PATH))
...
def gcs_upload(local_path, gcs_path, project_id=None, force=False):
    ...
    try:
        ...
        if "BucketNotFoundException" in result:
            raise ValueError(
                "ERROR: bucket not found, path={}".format(bucket_name))
        if result and not force:
            raise Warning(
                "WARNING: gcs file already exists, use force=True. bucket={}".
                format(bucket_name))
...
def load_from_bucket(tar_filename, bucket, train_dir):
  ...
  """
    ...
    if not found:
        raise ValueError(
            "ERROR: tar.gz file not found in bucket, path={}".format(
                bucket_path))
    ...
    if not os.path.isfile(tar_filepath):
        ...
        print("WARNING: using existing tar.gz file, path={}".format(
            tar_filepath))
    ...
    get_ipython().system_raw("tar -xzvf {} --overwrite --directory {} ".format(
        tar_filepath, train_dir))
    ...
    if global_step:
        checkpoint_name = os.path.join(train_dir, "model.ckpt-{}".format(
            global_step[0]))
    else:
        raise RuntimeError(
            "cannot get checkpoint from tar_filename, path={}".format(
                tar_filename))
...
def load_latest_checkpoint_from_bucket(tensorboard_run, bucket, train_dir):
  ...
  """
    ...
    if not checkpoints:
        raise ValueError(
            "Checkpoint not found, tensorboard_run={}".format(tensorboard_run))
    ...
    if not latest_step:
        raise ValueError(
            "Checkpoint not found, tensorboard_run={}".format(tensorboard_run))
    latest_checkpoint = [
        f for f in checkpoints if latest_step.astype(str) in f
    ]
    print("latest checkpoint found, checkpoint={}".format(
        latest_checkpoint[0]))
...
def save_to_bucket(train_dir,
  ...
  """
    ...
    """
        ...
        try:
            dirs_subdirs = [dir] + [
                os.path.join(dir, d) for d in next(os.walk(dir))[1]
                if not d.startswith(".")
            ]
            for d in dirs_subdirs:
                f += [
                    os.path.join(d.replace(dir, '.'), f) for f in os.listdir(d)
                    if not f.startswith(".")
                ]
            return [f, dir]
        except:
    ...
    if global_step:
        ...
        if found and not force:
            raise RuntimeError(
                "WARNING: a tar.gz file already exists, path={}. use force=True to overwrite".
                format(found[0]))
        ...
        print("writing tar.gz archive to, file={}, count={} ...".format(
            tar_filepath, len(filelist)))
        ...
        result = get_ipython().system_raw("tar -czvf {} -C {} {}".format(
            tar_filepath, checkpoint_path, " ".join(filelist)))

        if not os.path.isfile(tar_filepath):
            raise RuntimeError(
                "ERROR: tar file not created, path={}".format(tar_filepath))
        ...
        if type(result) == dict and result['err_code']:
            raise RuntimeError(
                "ERROR: error uploading to gcloud, bucket={}".format(
                    bucket_path))

        print("saved: tar={} \n> bucket={} \n> files={}".format(
            os.path.basename(tar_filepath), bucket_path, files))
        return bucket_path
    else:
...
def gcsfuse(bucket=None,
  ...
  """
    ...
    if not found:
        ...
        GCSFUSE_REPO = "gcsfuse-{}".format(
            __shell__(_cmd["get_lsb_release"], split=False))
        # add package to distro
        line_entry = "deb http://packages.cloud.google.com/apt {} main".format(
            GCSFUSE_REPO)
    ...
    if not tf.gfile.Exists(FUSED_BUCKET_PATH):
        tf.gfile.MakeDirs(FUSED_BUCKET_PATH)
      ...
      """
        ...
        if self._callback_op is not None:
            ...
            try:
                ...
                print(
                    "WARNING: SaverWithCallback() callback exception, err={}".
                    format(e))
